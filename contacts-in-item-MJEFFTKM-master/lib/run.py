...
class Test:
    ...
    def process(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        test = self.test()
        test = test.drop(['subcategory',
                          'category',
                          'price',
                          'region',
                          'city',
                          'datetime_submitted'], axis=1)

        task1_prediction = pd.DataFrame(columns=['index', 'prediction'])
        indices, preds = task1(test)
        task1_prediction['index'], task1_prediction['prediction'] = indices, preds


        task2_prediction = pd.DataFrame(columns=['index', 'start', 'finish'])
        task2_prediction['index'], task2_prediction['start'], task2_prediction['finish'] = task2(test['description'], indices, preds)
